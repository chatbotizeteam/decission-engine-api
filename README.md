...
Finishes the current block with the given status.
This function does not have to be explicitly called in most cases, as the block will automatically finish when Javascript evaluation completes.
...
### interaction.getPlaintextTranscript({ timeFrom?: number; timeTo?: number; timezoneOffsetMinutes?: number; hideRealAgentNames?: boolean; }): string
...
### interaction.getHtmlTranscript({ timeFrom?: number; timeTo?: number; timezoneOffsetMinutes?: number; hideRealAgentNames?: boolean; }): string
...
### crm.getUser(): Record<string, any>
Retrieves the current user's information
Contains data written to the CRM earlier via write/writeMany, as well as default parameters that exist on the User object and parameters set by other parts of the system
...
| refundLineItems | { itemId: string; quantity: number; restockType: "CANCEL" \| "RETURN" \| "NO\_RESTOCK"; }[] | Line items to be refunded (default: []) |
